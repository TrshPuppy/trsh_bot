...
const commands = {
    ...
    project: {
        exe: (c) => {
            server.say(
                apiData.Bot.CHANNEL,
                "Today we're hacking Forest on Hack the Box --> https://app.hackthebox.com/machines/212"
            );
            return;
        },
        manual: () => {
    ...
    socials: {
        exe: (c) => {
            server.say(
                apiData.Bot.CHANNEL,
                "Twitter -> twitter.com/trshpuppy, YouTube -> youtube.com/@trshpuppy, GitHub -> github.com/trshpuppy, WebSite -> TrshPuppy.github.io"
            );
        },
        manual: () => {
