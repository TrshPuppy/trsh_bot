...
const commands = {
  ...
  prompt: {
    exe: async (context) => {
      const success = await handleAddPrompt(context);
      if (success) {
        server.say(
          apiData.Bot.CHANNEL,
  ...
  getprompt: {
    exe: async (context) => {
      ...
      if (context.tags["mod"] === false) {
        return;
      }

      // If they're a mod, let em cook:
      try {
        const nextPromptInQueue = await getPromptFromDB(context.db); // returns an object including .prompt and .author keys
      ...
      try {
        await markPromptIncomplete(context.db, nextPromptInQueue.rowid);
      } catch (err) {
...
async function handleAddPrompt(context) {
  ...
  const wasThePromptAddSuccessful = await addPrompt(context.db, promptObj);
